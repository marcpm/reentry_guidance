...
from .util import geometric_to_geopotential
...
def table(x, kind="geopotential"):
    ...
    if kind == "geopotential":
        ...
        alt = geometric_to_geopotential(x)
    else:
    ...
    if (h < H[0]).any() or (h > H[-1]).any():
        ...
        return 0,0,0,2.2685010e-5
        # return 0,0,0,0
        raise ValueError(
        f"the given altitude x= {h} is out of bound, this module is "
